...
namespace aodv
{
    ...
    void Node::send(Eth &eth, subt::CommsClient* commsClient, bool isOriginating)
    {
        ...
        const uint16_t maxPayloadLength = aodv::MAX_MESSAGE_SIZE - (aodv::ETH_NONVAR_LEN + seg.srcLength + seg.dstLength);
        ...
        for (; p<eth.payloadLength; segSeq++,p+=maxPayloadLength) {
            ...
            seg.payload = seg.payload.substr(p, p + seg.payloadLength);
            seg.segSeq = segSeq;
