...
const uint8_t ACTIVE_ROUTE_TIMEOUT = 3000; // ms // If Hello messages are used, then the ACTIVE_ROUTE_TIMEOUT parameter value MUST be more than the value (ALLOWED_HELLO_LOSS * HELLO_INTERVAL).
const uint8_t ALLOWED_HELLO_LOSS = 2;
const uint8_t HELLO_INTERVAL = 1000; // ms
...
// Derived constants
const uint8_t DELETE_PERIOD = K * std::max(ACTIVE_ROUTE_TIMEOUT, HELLO_INTERVAL);
const uint8_t MAX_REPAIR_TTL = 0.3 * NET_DIAMETER;
const uint8_t MY_ROUTE_TIMEOUT = 2 * ACTIVE_ROUTE_TIMEOUT;
const uint8_t NET_TRAVERSAL_TIME = 2 * NODE_TRAVERSAL_TIME * NET_DIAMETER;
const uint8_t NEXT_HOP_WAIT = NODE_TRAVERSAL_TIME + 10;
const uint8_t PATH_DISCOVERY_TIME = 2 * NET_TRAVERSAL_TIME;
const uint8_t RING_TRAVERSAL_TIME = 2 * NODE_TRAVERSAL_TIME * (TTL_VALUE + TIMEOUT_BUFFER);
const uint8_t BLACKLIST_TIMEOUT = RREQ_RETRIES * NET_TRAVERSAL_TIME;
